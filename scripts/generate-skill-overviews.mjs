...
const skills = [
  {id:'comics-shorts', title:'Comics Shorts', category:'Формат Shorts', summary:'Фотографические сцены, комиксные реплики и движение камеры по истории.', when:'Нужен короткий рассказ в виде последовательности реалистичных кадров с комиксными облачками.', input:'Аудио или сценарий, герои и визуальные ограничения.', result:'Проверенный пакет кадров для ревью, затем смонтированный вертикальный ролик.', steps:['Разбить рассказ на сцены','Подготовить фото и реплики','Показать кадры на ревью'], color:'#ffb454', example:'assets/session-example.png', exampleCaption:'Подготовленный фотографический кадр из проекта Vasco da Gama (Comics Shorts).'},
  {id:'cringe-meme', title:'Cringe Meme', category:'Вспомогательный', summary:'Подбор подходящей кринжовой или неловкой реакции из локальной коллекции смайликов.', when:'Нужна реакционная картинка для мема, монтажа или короткого видео.', input:'Сцена, эмоция или желаемый тип реакции.', result:'Выбранный файл смайлика с названием и путём.', steps:['Описать нужную реакцию','Найти кандидатов в каталоге','Выбрать и показать смайлик'], color:'#ff6b81', example:'assets/smiles/1682570821_papik-pro-p-smail-krinzh-png-2.png', exampleCaption:'Конкретный ассет из подборки cringe-meme — пример одной из реакций.'},
  {id:'doodle-jump', title:'Doodle Jump', category:'Формат Shorts', summary:'Рассказанный Short как подъём героя по платформам через единый движущийся мир.', when:'История подходит для последовательного подъёма, открытий и препятствий.', input:'Озвучка, сценарий, герой и визуальные материалы.', result:'Редактируемый Remotion-проект, MP4 и кадры для QA.', steps:['Разметить фразы и события','Собрать героя, мир и платформы','Проверить маршрут и рендер'], color:'#71d6a4', example:'assets/session-example.png', exampleCaption:'Кадр из финального Doodle Jump Shorts: герой поднимается по платформам.'},
  {id:'paint-full-shorts', title:'Paint Full Shorts', category:'Формат Shorts', summary:'Ретро-редактор становится сценой: курсор, слои и инструменты рассказывают историю.', when:'Смысл ролика можно раскрыть через видимые операции редактирования.', input:'Идея или озвучка, изображения и желаемая операция.', result:'Редактируемый Short с понятной последовательностью действий.', steps:['Выбрать действие редактора','Показать инструмент и изменение','Проверить читаемость результата'], color:'#70a7ff', example:'assets/session-example.png', exampleCaption:'Кадр из финального ролика о лотерее Головина: Photoshop-подобный редактор как сцена истории.'},
  {id:'paint95-video-maker', title:'Paint95 Video Maker', category:'Формат Shorts', summary:'Образовательный коллаж в окне Windows 95 Paint с покадровым движением и озвучкой.', when:'Нужен ролик в установленной визуальной стилистике Paint95.', input:'Сценарий или готовое аудио, источники и стиль.', result:'Проверенный MP4, проект, субтитры и контакт-лист.', steps:['Сопоставить речь и визуальные beats','Собрать Paint95-коллаж','Проверить proxy и финал'], color:'#53c8d0', example:'assets/session-example.png', exampleCaption:'Кадр из проекта Mitsubishi в стиле Paint95 — пример визуального результата.'},
