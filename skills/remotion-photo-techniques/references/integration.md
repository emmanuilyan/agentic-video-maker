...
| `text` | Надпись TEXT_PUSH/TEXT_REVEAL/TEXT_HIGHLIGHT. Для TEXT_HIGHLIGHT разделяй строки `\n`. Проверяй длинные слова и новый шрифт. |
| `accent` | Цвет акцентов, включая текст, рамки и маркер совпадения. У TEXT_HIGHLIGHT значение по умолчанию `#E9D642`; у остальных приёмов — `#d8ed78`. |
...
Для жёлтого выделения поверх существующего видео скопируй только `assets/techniques/text-highlight.tsx` и импортируй `YellowTextHighlight` прямо из файла: ему нужны лишь React и Remotion, без `PhotoTechnique`, массива фото и 3D-зависимостей. Если вся библиотека уже подключена, компонент также экспортируется из `index.tsx`. Точные параметры и пример — в [TEXT_HIGHLIGHT](text-highlight.md).
