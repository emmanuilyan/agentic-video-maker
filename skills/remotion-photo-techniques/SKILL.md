...
description: Use when animating photographs in Remotion with grids, zooms, rolling columns, card stacks, collages, photo transitions, kinetic text, impact effects, parallax, or camera flythroughs; also when a named photo technique needs its own preview.
---

# Remotion Photo Techniques

Применяй 29 именованных приёмов монтажа фотографий в Remotion, включая 8 пространственных сцен с камерой. Сохраняй точные идентификаторы из [каталога](references/techniques.md), чтобы запрос «сделай GRID_ZOOM» однозначно выбирал эффект.
...
3. Библиотека — готовые демонстрационные сцены на **четырёх фото**, а не универсальный transition API. Для другой численности или встраивания одной склейки адаптируй выбранный приём по интеграции. Не подменяй входные фотографии ради ограничения примера.
