...
When the user asks for a rendered video, include one representative PNG still from the final composition alongside the video. If rendering was not requested, do not render just to make a PNG; use an already available Studio preview only when a visual preview is useful.
