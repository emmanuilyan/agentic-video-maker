...
For visual video analysis, include a small PNG contact sheet or one timestamped representative frame when it materially supports the answer. Convert only the selected existing JPEG frames; do not extract extra frames just to create a preview. For transcript-only questions, omit the image.
